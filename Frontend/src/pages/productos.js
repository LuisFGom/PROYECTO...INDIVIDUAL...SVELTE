...
export class Productos {
  ...
  render() {
    ...
    return `
<div class="productos-page" style="padding: 20px;">
  <div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
    <h1 class="page-title" style="font-size: 28px; color: #1E293B; margin: 0;"><i class="fas fa-box"></i> Productos</h1>
    ${isAdmin ? `
      <button class="btn-add-producto" title="Crear nuevo producto" style="padding: 10px 20px; background: #3B82F6; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
      ...
      <div class="table-container" style="overflow-x: auto;">
        <table class="table" style="width: 100%; border-collapse: collapse;">
          ...
          <tbody id="productos-tbody">
            <tr><td colspan="${isAdmin ? 6 : 5}" style="padding: 40px; text-align: center; color: #64748B;">Cargando...</td></tr>
          </tbody>
        </table>
      </div>
    ...
    try {
      ...
      if (Array.isArray(productos)) {
        ...
        this.productos = productos.sort((a, b) => {
          // Si tienen fechaCreacion, usar eso
  ...
  renderTable() {
    ...
    if (paginatedProductos.length === 0) {
      tbody.innerHTML = `<tr><td colspan="${isAdmin ? 8 : 7}" style="padding: 40px; text-align: center; color: #64748B;">${this.productos.length === 0 ? 'No hay productos registrados' : 'No se encontraron resultados'}</td></tr>`
      return
    }
    ...
    paginatedProductos.forEach(producto => {
      ...
      html += `<tr style="border-bottom: 1px solid #E2E8F0;">
        ...
        <td style="padding: 15px; text-align: right; color: #1E293B;">$${(producto.precio || 0).toFixed(2)}</td>
  ...
  async saveProducto(e) {
    ...
    if (!nombre || isNaN(precioVenta) || isNaN(precioCosto)) {
      Swal.fire({ icon: 'warning', title: 'Campos requeridos', text: 'Por favor completa Nombre, Precio de Venta y Precio de Costo.', confirmButtonColor: '#3B82F6' })
      return
    }
    ...
    if (stockMinimo > stockActual) {
      Swal.fire({ icon: 'warning', title: 'Stock Inválido', text: `Stock mínimo (${stockMinimo}) no puede ser mayor que stock actual (${stockActual}).`, confirmButtonColor: '#3B82F6' })
      return
    }
    ...
    try {
      if (this.editingId) {
        ...
        if (!codigoBarra) {
          Swal.fire({ icon: 'warning', title: 'Campo requerido', text: 'El código de barra es requerido.', confirmButtonColor: '#3B82F6' })
          return
        }
      ...
      Swal.fire({ icon: 'error', title: 'Error al guardar', text: errorMessage, confirmButtonColor: '#EF4444' })
  ...
  async editProducto(id) {
    ...
    if (!producto) {
      Swal.fire({ icon: 'error', title: 'Error', text: 'Producto no encontrado.', confirmButtonColor: '#EF4444' })
      return
    }
