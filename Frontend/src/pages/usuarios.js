...
export class Usuarios {
  ...
  render() {
    ...
    return `
<div class="usuarios-page" style="padding: 20px;">
  <div class="page-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 30px;">
    <h1 class="page-title" style="font-size: 28px; color: #1E293B; margin: 0;"><i class="fas fa-users"></i> Gestión de Usuarios</h1>
    <button class="btn-add-usuario" title="Crear nuevo usuario" style="padding: 10px 20px; background: #3B82F6; color: white; border: none; border-radius: 8px; cursor: pointer; font-weight: 600;">
  ...
  renderTable() {
    ...
    if (paginatedUsuarios.length === 0) {
      tbody.innerHTML = `<tr><td colspan="7" style="padding: 40px; text-align: center; color: #64748B;">${this.usuarios.length === 0 ? 'No hay usuarios registrados' : 'No se encontraron resultados'}</td></tr>`
      return
    }
  ...
  async editUsuario(id) {
    ...
    if (!usuario) {
      Swal.fire({ icon: 'error', title: 'Error', text: 'Usuario no encontrado.', confirmButtonColor: '#EF4444' })
      return
    }
