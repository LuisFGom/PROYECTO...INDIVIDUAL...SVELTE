...
namespace PuntoVenta.Api.Controllers
{
    ...
    public class UsuariosController : ControllerBase
    {
        ...
        public UsuariosController(IUnitOfWork unitOfWork, IMediator mediator, IEmailValidationService emailValidationService)
        {
        ...
        public async Task<IActionResult> GetUsuarios()
        {
            try
            {
                // Debug: loguear rol del usuario actual
                var userRole = User.FindFirst(System.Security.Claims.ClaimTypes.Role)?.Value ?? "NO_ROLE";
                System.Console.WriteLine($"[DEBUG] User Role in JWT: {userRole}");
                ...
                var usuariosDto = usuarios.Select(u => new UsuarioResponseDto
                {
                    ...
                    Cedula = u.NombreUsuario, // Alias para el frontend
                    Email = u.Email,
                    Correo = u.Email, // Alias para el frontend
                    Nombre = u.Nombre,
                    Apellido = u.Apellido,
                    Activo = u.Activo,
        ...
        public async Task<IActionResult> GetUsuario(int id)
        {
            try
            {
                ...
                var usuarioDto = new UsuarioResponseDto
                {
                    ...
                    Cedula = usuario.NombreUsuario, // Alias para el frontend
                    Email = usuario.Email,
                    Correo = usuario.Email, // Alias para el frontend
        ...
        public async Task<IActionResult> CreateUsuario([FromBody] CreateUsuarioRequestDto createDto)
        {
            ...
            try
            {
                ...
                // Validar que el email es único si se proporciona
                var email = createDto.Email ?? createDto.Correo;
                if (!string.IsNullOrWhiteSpace(email))
                {
                    var isEmailUnique = await _emailValidationService.IsEmailUniqueAsync(email);
                    if (!isEmailUnique)
                    {
                        var owner = await _emailValidationService.GetEmailOwnerAsync(email);
        ...
        public async Task<IActionResult> UpdateUsuario(int id, [FromBody] UpdateUsuarioDto updateUsuarioDto)
        {
            ...
            try
            {
                ...
                // Validar que el email es único si se proporciona y es diferente
                if (!string.IsNullOrEmpty(updateUsuarioDto.Email) && 
                    updateUsuarioDto.Email != usuario.Email)
                {
                    var isEmailUnique = await _emailValidationService.IsEmailUniqueAsync(
                        updateUsuarioDto.Email, 
                        excludeUserId: id
                    );
                    if (!isEmailUnique)
                    {
                        var owner = await _emailValidationService.GetEmailOwnerAsync(updateUsuarioDto.Email);
                        return BadRequest(new { message = $"El correo ya está registrado por: {owner}" });
                    }
                }
        ...
        /// <summary>
        /// Verifica si un email es único en el sistema (usuarios y clientes)
        /// </summary>
        ...
        public async Task<ActionResult<object>> CheckEmailUnique([FromBody] EmailCheckRequest request)
        {
            try
            {
                ...
                var isUnique = await _emailValidationService.IsEmailUniqueAsync(
                    request.Email, 
                    excludeUserId: request.ExcludeId
                );
