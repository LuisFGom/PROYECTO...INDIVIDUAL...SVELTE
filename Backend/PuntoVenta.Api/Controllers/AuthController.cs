...
namespace PuntoVenta.Api.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> DebugAdmin()
        {
            try
            {
                var usuario = await _unitOfWork.Usuarios.GetByCorreoAsync("admin@test.com");
                if (usuario == null)
                {
                    return Ok(new { mensaje = "Usuario admin no encontrado" });
                }
                ...
                return Ok(new
                {
                    ...
                    passwordHashExists = !string.IsNullOrEmpty(usuario.PasswordHash),
                    passwordHashLength = usuario.PasswordHash?.Length ?? 0,
                    passwordHashPreview = usuario.PasswordHash?.Substring(0, Math.Min(50, usuario.PasswordHash?.Length ?? 0)) ?? ""
                });
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Registro de nuevo usuario (permitir auto-registro con rol defecto)
        /// </summary>
        [HttpPost("registro")]
        [AllowAnonymous]
        public async Task<IActionResult> Registro([FromBody] CreateUsuarioDto createUsuarioDto)
        {
            ...
            try
            {
                // Buscar o crear el rol "defecto"
                var roles = await _unitOfWork.Roles.GetAllAsync();
                var rolDefecto = roles.FirstOrDefault(r => r.Nombre.Equals("Defecto", StringComparison.OrdinalIgnoreCase));
                
                if (rolDefecto == null)
                {
                    // Si no existe, crear el rol "defecto"
                    rolDefecto = new Rol
                    {
                        Nombre = "Defecto",
                    ...
                    await _unitOfWork.Roles.AddAsync(rolDefecto);
