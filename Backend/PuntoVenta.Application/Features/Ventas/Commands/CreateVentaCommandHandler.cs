...
namespace PuntoVenta.Application.Features.Ventas.Commands
{
    public class CreateVentaCommandHandler : IRequestHandler<CreateVentaCommand, int>
    {
        ...
        public CreateVentaCommandHandler(IUnitOfWork unitOfWork, IEmailService emailService, ILogger<CreateVentaCommandHandler> logger)
        {
        ...
        public async Task<int> Handle(CreateVentaCommand request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                // Enviar email de confirmación de venta si el cliente tiene email
                if (!string.IsNullOrWhiteSpace(factura.ClienteNombre) && factura.ClienteId.HasValue)
                {
                    var cliente = await _unitOfWork.Clientes.GetByIdAsync(factura.ClienteId.Value);
                    if (cliente != null && !string.IsNullOrWhiteSpace(cliente.Email))
                    {
                        ...
                        {
                            _logger.LogWarning($"⚠️ No se pudo enviar email de venta a {cliente.Email}: {emailEx.Message}");
                        }
                    }
                }
